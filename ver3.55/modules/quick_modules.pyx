...
cdef ret_grid_list_teams(grid_list, grid, num):
	return [grid2 for grid2 in grid_list if grid2.teams[num].name == grid.teams[num].name]

cdef grid_list2square_list(grid_list):
	#cdef unsigned long square[2]
	square_list = []
...
cdef square2grid(square, grid_list):
...
cdef team_code_from_square(square, x):
	if x == 0:
...
cdef comparison1_from_square(square):
	return (square[0] >> (1))&(0b11111)

cdef comparison2_from_square(square):
	return (square[0] >> (6))&(0b11111)

cdef square_available(square):
	if square[0]&1 == 1:
		return True
	else:
		return False

cdef ret_same_team_code_square_list(square_list, code):
