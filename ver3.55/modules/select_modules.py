...
def select_grid2(grid_list, team_list):#####????work weel????
	if len(grid_list[0].teams) == 2:
		grid = find_min_grid(grid_list)
		grid_list_team0s = [grid2 for grid2 in grid_list if grid2.teams[0] == grid.teams[0]]
		grid_list_team1s = [grid1 for grid1 in grid_list if grid1.teams[1] == grid.teams[1]]
		...
		grid = find_min_grid(grid_list)
		grid_list_team0s = [grid2 for grid2 in grid_list if grid2.teams[0] == grid.teams[0]]
		grid_list_team1s = [grid2 for grid2 in grid_list if grid2.teams[1] == grid.teams[1]]
		grid_list_team2s = [grid2 for grid2 in grid_list if grid2.teams[2] == grid.teams[2]]
		grid_list_team3s = [grid2 for grid2 in grid_list if grid2.teams[3] == grid.teams[3]]
		...
		teams_dict_sorted = sorted(teams_dict.items(), key=lambda x:x[1], reverse=True)
	...
	for grid in grid_list:
		if grid.grid_type() == "Grid":
			if len(grid_list[0].teams) == 2:
				...
				elif grid.teams[0] != grid.teams[1]:
					grid.available = True
			else:
			...
			if len(grid_list[0].grid.teams) == 2:
				if not(grid.grid.teams[0].available) or not(grid.grid.teams[1].available):
					grid.available = False
				elif not grid.chair.absent:
					grid.available = True
			else:
				if not(grid.grid.teams[0].available) or not(grid.grid.teams[1].available) or not(grid.grid.teams[2].available) or not(grid.grid.teams[3].available):
					grid.available = False
				elif not grid.chair.absent:
					grid.available = True
...
def select_alg3(grid_list, round_num, team_list):#picking up by disavailing the worse ones(by team)
	if len(grid_list[0].teams) == 2:
		...
		#print selected_grid_list
		"""
		return select_alg3_high(grid_list, team_list)
