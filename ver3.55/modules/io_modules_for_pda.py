...
def export_blank_results(allocations, round_num):
	with open("blankresults/Results"+str(round_num)+".csv", "w") as g:
		writer = csv.writer(g)
		writer.writerow(["team_name", "name", "R"+str(round_num)+" 1st", "R"+str(round_num)+" 2nd", "R"+str(round_num)+" 3rd", "Win[0, 1]", "Opponent", "Gov Opp[gov, opp]"])
		...
		for lattice in allocations:
			...
			writer.writerow([lattice.grid.teams[0].name, gov_members[2].name, 0, 0, 0, "", lattice.grid.teams[1].name, ""])
			...
			writer.writerow([lattice.grid.teams[1].name, opp_members[2].name, 0, 0, 0, "", lattice.grid.teams[0].name, ""])
	...
	for i in range(int(len(results_list)/3)):#results=>[team name, name, R[i] 1st, R[i] 2nd, R[i] rep, win?lose?, opponent name, gov?opp?]
		for team in team_list:
			if team.name == results_list[3*i][0]:
				...
				member1_score_list = results_list[3*i][2:5]
				member2_score_list = results_list[3*i+1][2:5]
				member3_score_list = results_list[3*i+2][2:5]
				side = results_list[3*i][7]
				win = results_list[3*i][5]
				...
				for results in results_list:
					if results[0] == results_list[3*i][6]:
...
def export_random_result(allocations, round_num):
	with open("private/Results"+str(round_num)+".csv", "w") as g:
		writer = csv.writer(g)
		writer.writerow(["team_name", "name", "R"+str(round_num)+" 1st", "R"+str(round_num)+" 2nd", "R"+str(round_num)+" 3rd", "Win", "Opponent", "Gov Opp"])
		...
		for lattice in allocations:
			...
			writer.writerow([lattice.grid.teams[0].name, gov_members[0].name, ga_score, 0, 0, team1_win, lattice.grid.teams[1].name, "gov"])
			writer.writerow([lattice.grid.teams[0].name, gov_members[1].name, 0, gb_score, 0, team1_win, lattice.grid.teams[1].name, "gov"])
			writer.writerow([lattice.grid.teams[0].name, gov_members[2].name, 0, 0, gc_score, team1_win, lattice.grid.teams[1].name, "gov"])
			writer.writerow([lattice.grid.teams[1].name, opp_members[0].name, oa_score, 0, 0, 1-team1_win, lattice.grid.teams[0].name, "opp"])
			writer.writerow([lattice.grid.teams[1].name, opp_members[1].name, 0, ob_score, 0, 1-team1_win, lattice.grid.teams[0].name, "opp"])
			writer.writerow([lattice.grid.teams[1].name, opp_members[2].name, 0, 0, oc_score, 1-team1_win, lattice.grid.teams[0].name, "opp"])
...
def export_results(team_list, adjudicator_list):
	...
	for i in range(int((len(individual_score_rows[0])-2)/4)):
		header.extend(["round"+str(i+1)+" 1st", "round"+str(i+1)+" 2nd", "round"+str(i+1)+" 3rd", "round"+str(i+1)+" average"])
