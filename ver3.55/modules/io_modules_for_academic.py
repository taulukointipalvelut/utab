...
def export_blank_results(allocations, round_num):
	with open("blankresults/Results"+str(round_num)+".csv", "w") as g:
		writer = csv.writer(g)
		writer.writerow(["team_name", "name", "R"+str(round_num)+" const", "R"+str(round_num)+" attack", "R"+str(round_num)+" defense", "R"+str(round_num)+" summary", "Win[0, 1]", "Opponent", "Gov Opp[gov, opp]"])
		...
		for lattice in allocations:
			...
			writer.writerow([lattice.grid.teams[0].name, gov_members[3].name, 0, 0, 0, 0, "", lattice.grid.teams[1].name, ""])
			...
			writer.writerow([lattice.grid.teams[1].name, opp_members[3].name, 0, 0, 0, 0, "", lattice.grid.teams[0].name, ""])
...
def read_and_process_result(round_num, team_list):
	...
	for i in range(int(len(results_list)/4)):#results=>[team name, name, R[i] 1st, R[i] 2nd, R[i] rep, win?lose?, opponent name, gov?opp?]
		for team in team_list:
			if team.name == results_list[4*i][0]:
				member1_name = results_list[4*i][1]
				member2_name = results_list[4*i+1][1]
				member3_name = results_list[4*i+2][1]
				member4_name = results_list[4*i+3][1]
				member1_score_list = results_list[4*i][2:6]
				member2_score_list = results_list[4*i+1][2:6]
				member3_score_list = results_list[4*i+2][2:6]
				member4_score_list = results_list[4*i+3][2:6]
				side = results_list[4*i][8]
				win = results_list[4*i][6]
				for debater in team.debaters:
					...
					elif debater.name == member4_name:
						debater.score_lists.append(member4_score_list)
						debater.scores.append(sum(member4_score_list))
					else:
						print "error: Results file(Results"+str(round_num)+".csv) broken"

				opp_team_score = 0
				for results in results_list:
					if results[0] == results_list[4*i][7]:
						opp_team_score += sum(results[2:6])

				team.finishing_process(opponent=[results_list[4*i][6]], score=sum(member1_score_list)+sum(member2_score_list)+sum(member3_score_list)+sum(member4_score_list), side=side, win=win, margin=sum(member1_score_list)+sum(member2_score_list)+sum(member3_score_list)+sum(member4_score_list)-opp_team_score)
...
def export_random_result(allocations, round_num):
	with open("private/Results"+str(round_num)+".csv", "w") as g:
		writer = csv.writer(g)
		writer.writerow(["team_name", "name", "R"+str(round_num)+" const", "R"+str(round_num)+" attack", "R"+str(round_num)+" defense", "R"+str(round_num)+" summary", "Win", "Opponent", "Gov Opp"])

		for lattice in allocations:
			ga_base_score = random.randint(73, 77)
			gb_base_score = random.randint(73, 77)
			gc_base_score = random.randint(73,77)
			gd_base_score = random.randint(73,77)
			oa_base_score = random.randint(73, 77)
			ob_base_score = random.randint(73, 77)
			oc_base_score = random.randint(73,77)
			od_base_score = random.randint(73,77)
			gov_members = copy.copy(lattice.grid.teams[0].debaters)
			opp_members = copy.copy(lattice.grid.teams[1].debaters)
			...
			gc_score_from_chair = gd_base_score + random.randint(-2,+2)
			gd_score_from_panel1 = gd_base_score + random.randint(-2,+2)
			gd_score_from_panel2 = gd_base_score + random.randint(-2,+2)
			gd_score_from_chair = gd_base_score + random.randint(-2,+2)
			...
			od_score_from_panel1 = od_base_score + random.randint(-2,+2)
			od_score_from_panel2 = od_base_score + random.randint(-2,+2)
			od_score_from_chair = od_base_score + random.randint(-2,+2)
			...
			writer.writerow([lattice.grid.teams[0].name, gov_members[3].name, 0, 0, 0, gd_score, team1_win, lattice.grid.teams[1].name, "gov"])
			...
			writer.writerow([lattice.grid.teams[1].name, opp_members[3].name, 0, 0, 0, od_score, 1-team1_win, lattice.grid.teams[0].name, "opp"])
...
def read_teams(filename):
	...
	for code, raw_row in enumerate(reader):
		row = [raw_one.strip() for raw_one in raw_row]
		if row[0] != '':
			teams.append(Team(code, row[0], [row[1], row[2], row[3], row[4]], row[5], row[6:]))#=>code, name, member_names
	return teams
...
def export_results(team_list, adjudicator_list):
	...
	for team in team_list:
		for debater in team.debaters:
			...
			for score_list in debater.score_lists:
				...
				row.append(round(sum(score_list), 2))#####add average score for each round
				sum_score += sum(score_list)
			if len(debater.score_lists) == 0:
				average_score = 0
			else:
				average_score = round(sum_score/(len(debater.score_lists)), 2)
	...
	for i in range(int((len(individual_score_rows[0])-2)/5)):
		header.extend(["round"+str(i+1)+" const", "round"+str(i+1)+" attack", "round"+str(i+1)+" defense", "round"+str(i+1)+" summary", "round"+str(i+1)+" average"])
...
def export_teams(filename_teams, team_list):
	with open(filename_teams, "w") as g:
		writer = csv.writer(g)
		writer.writerow(["team name", "name1", "mame2", "name3", "name4", "scale", "institution"])
		for team in team_list:
			export_row = [team.name, team.debaters[0].name, team.debaters[1].name, team.debaters[2].name, team.debaters[3].name, team.institution_scale]
...
def check_results(filename):
	...
	for i in range(int(len(results_lists)/4)):
		if results_lists[4*i][0] != results_lists[4*i+1][0] or results_lists[4*i][0] != results_lists[4*i+1][0] or results_lists[4*i0][0] != results_lists[4*i+2][0] or results_lists[4*i][0] != results_lists[4*i+3][0] or results_lists[4*i+1][0] != results_lists[4*i+2][0] or results_lists[4*i+1][0] != results_lists[4*i+3][0] or results_lists[4*i+2][0] != results_lists[4*i+3][0]:
			print("error in team name column, row:"+str(4*i+3))
			return True
	...
	try:
		...
		for row in reader:
			for team in team_list:
				...
				new_team = Team(len(team_list), row[0], [row[2], row[3], row[4], row[5]], row[6], row[7:])
