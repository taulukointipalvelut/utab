...
def select_lattice2(lattice_list, team_list, cp_pair):
	lattice = find_min_grid(lattice, cp_pair)
	...
	max_in_row, grid_row = find_max_grid(lattice_list_adjs, cp_pair)
	max_in_column, grid_column = find_max_grid(lattice_list_grids, cp_pair)
...
def select_grid2(grid_list, team_list, cp_pair):#####????work well????
	...
	for grid_list_teams in grid_list_teams_list:
		max_in_xs, grid_xs = find_max_grid(grid_list_teams, cp_pair)
...
def find_min_grid(grid_list, cp_pair):
...
def select_alg2(grid_list, round_num, team_list, cp_pair):#!!#picking up from the highly desired
	...
	while len_available(grid_list):#complete selected grids list""
		grid = select_grid2(grid_list, team_list, cp_pair)
		selected_grid_list.append(grid)
		disavail_grids(grid_list, grid)
...
def select_alg3(grid_list, round_num, team_list, cp_pair):#picking up by disavailing the worse ones(by team)
	selected_grid_list = []
...
def select_alg4(grid_list, round_num, team_list, cp_pair):#picking up by disavailing the worse ones(by row/column)
	if len(grid_list[0].teams) == 2:
		selected_grid_list = []
		...
		while len_available(grid_list) > 0:
			...
			max_in_row, grid_row = find_max_grid(grid_list_team2s, cp_pair)
			max_in_column, grid_column = find_max_grid(grid_list_team1s, cp_pair)
		...
		while len_available(grid_list) > 0:
			...
			max_in_0s, grid_0s = find_max_grid(grid_list_team2s, cp_pair)
			max_in_1s, grid_1s = find_max_grid(grid_list_team1s, cp_pair)
			max_in_2s, grid_2s = find_max_grid(grid_list_team2s, cp_pair)
			max_in_3s, grid_3s = find_max_grid(grid_list_team1s, cp_pair)
...
def select_alg11(grid_list, round_num, team_list, cp_pair):#!!#picking up from the highly desired(2 criteria)
	selected_grid_list = []
	available_team_num = 0
	for team in team_list:
		if team.available: available_team_num+=1
...
def select_alg13(grid_list, round_num, team_list, cp_pair):#!!#picking up harmless ones
	selected_grid_list = []
	available_team_num = 0
	for team in team_list:
		if team.available: available_team_num+=1
	...
	grid_list.sort(key=lambda grid: (cp_pair[0](grid), -calc_deleting_effect(grid_list, grid)), reverse=True)
	for grid in grid_list:
		if grid.available:
			selected_grid_list.append(grid)
			disavail_grids(grid_list, grid)
...
def select_alg14(grid_list, round_num, team_list, cp_pair):#!!#picking up harmless ones(one criterion)
	selected_grid_list = []
	available_team_num = 0
	for team in team_list:
		if team.available: available_team_num+=1
...
def select_alg_adj2(lattice_list, round_num, team_list, cp_pair):#picking up from the highly desired
	...
	while len_available(lattice_list):#complete selected lattices list
		lattice = select_lattice2(lattice_list, cp_pair)
		selected_lattice_list.append(lattice)
		disavail_grids(lattice_list, lattice)
...
def select_alg_adj11(lattice_list, round_num, team_list, cp_pair):#picking up from the highly desired(2 criteria)
	selected_lattice_list = []
...
def select_alg_adj13(lattice_list, round_num, team_list, cp_pair):#picking up harmless ones
	selected_lattice_list = []

	#random.shuffle(lattice_list)
	lattice_list.sort(key=lambda lattice: (cp_pair[1](lattice), -calc_deleting_effect_adj(lattice_list, lattice)), reverse=True)
	for lattice in lattice_list:
		if lattice.available:
			selected_lattice_list.append(lattice)
			disavail_grids(lattice_list, lattice)
...
def select_alg_adj14(lattice_list, round_num, team_list, cp_pair):#picking up harmless ones(one criterion)
	selected_lattice_list = []
