...
def export_blank_results(allocations, round_num):
	with open("blankresults/Results"+str(round_num)+".csv", "w") as g:
		writer = csv.writer(g)
		writer.writerow(["team_name", "name", "R"+str(round_num)+" 1st", "R"+str(round_num)+" 2nd", "R"+str(round_num)+" 3rd", "R"+str(round_num)+" 4th", "Win[0, 1]", "Opponent", "Gov Opp[gov, opp]"])
	...
	for i in range(int(len(results_list)/2)):#results=>[team name, name, R[i] 1st, R[i] 2nd, R[i] rep, win?lose?, opponent name, gov?opp?]
		for team in team_list:
			if team.name == results_list[2*i][0]:
				...
				member1_score_list = results_list[2*i][2:6]
				member2_score_list = results_list[2*i+1][2:6]
				side = results_list[2*i][8]
				win = results_list[2*i][6]
				for debater in team.debaters:
					if debater.name == member1_name:
						debater.score_lists.append(member1_score_list)
						debater.scores.append(sum(member1_score_list)/2.0)
					elif debater.name == member2_name:
						debater.score_lists.append(member2_score_list)
						debater.scores.append(sum(member2_score_list)/2.0)
					else:
						print "error: Results file(Results"+str(round_num)+".csv) broken"

				opp_team_score = 0
				for results in results_list:
					if results[0] == results_list[2*i][7]:
						opp_team_score += sum(results[2:6])

				team.finishing_process(opponent=[results_list[2*i][7]], score=sum(member1_score_list)+sum(member2_score_list), side=side, win=win, margin=sum(member1_score_list)+sum(member2_score_list)-opp_team_score)
...
def export_random_result(allocations, round_num):
	with open("private/Results"+str(round_num)+".csv", "w") as g:
		writer = csv.writer(g)
		writer.writerow(["team_name", "name", "R"+str(round_num)+" 1st", "R"+str(round_num)+" 2nd", "R"+str(round_num)+" 3rd", "R"+str(round_num)+" 4th", "Win", "Opponent", "Gov Opp"])
		...
		for lattice in allocations:
			...
			od_score_from_panel1 = oc_base_score + random.randint(-2,+2)
			od_score_from_panel2 = oc_base_score + random.randint(-2,+2)
			od_score_from_chair = oc_base_score + random.randint(-2,+2)
			...
			writer.writerow([lattice.grid.teams[0].name, gov_members[0].name, ga_score, 0, gc_score, 0, team1_win, lattice.grid.teams[1].name, "gov"])
			...
			writer.writerow([lattice.grid.teams[1].name, opp_members[0].name, oa_score, 0, oc_score, 0, 1-team1_win, lattice.grid.teams[0].name, "opp"])
...
def export_results(team_list, adjudicator_list):
	...
	for i in range(int((len(individual_score_rows[0])-2)/5)):
		header.extend(["round"+str(i+1)+" 1st", "round"+str(i+1)+" 2nd", "round"+str(i+1)+" 3rd", "round"+str(i+1)+" 4th", "round"+str(i+1)+" average"])
