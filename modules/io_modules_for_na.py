...
def export_blank_results(allocations, round_num):
	with open("blankresults/Results"+str(round_num)+".csv", "w") as g:
		writer = csv.writer(g)
		writer.writerow(["team_name", "name", "R"+str(round_num)+" 1st", "R"+str(round_num)+" 2nd", "R"+str(round_num)+" rep", "Win[0, 1]", "Opponent", "Gov Opp[gov, opp]"])
		...
		for lattice in allocations:
			...
			writer.writerow([lattice.grid.teams[0].name, gov_members[0].name, 0, 0, 0, "", lattice.grid.teams[1].name, ""])
			writer.writerow([lattice.grid.teams[0].name, gov_members[1].name, 0, 0, 0, "", lattice.grid.teams[1].name, ""])
			writer.writerow([lattice.grid.teams[1].name, opp_members[0].name, 0, 0, 0, "", lattice.grid.teams[0].name, ""])
			writer.writerow([lattice.grid.teams[1].name, opp_members[1].name, 0, 0, 0, "", lattice.grid.teams[0].name, ""])
...
def read_results(filename):
	...
	results = []
	for row in reader:
		results.append([row[0], row[1], float(row[2]), float(row[3]), float(row[4]), int(row[5]), row[6], row[7]])
	return results
	...
	for i in range(int(len(results_list)/2)):#results=>[team name, name, R[i] 1st, R[i] 2nd, R[i] rep, win?lose?, opponent name, gov?opp?]
		for team in team_list:
			if team.name == results_list[2*i][0]:
				...
				member1_score_list = results_list[2*i][2:5]
				member2_score_list = results_list[2*i+1][2:5]
				side = results_list[2*i][7]
				win = results_list[2*i][5]
				for debater in team.debaters:
					if debater.name == member1_name:
						debater.score_lists.append(member1_score_list)
						if member1_score_list.count(0) == 2:
							debater.scores.append(sum(member1_score_list))
						elif member1_score_list.count(0) == 1:
						...
						if member2_score_list.count(0) == 2:
							debater.scores.append(sum(member2_score_list))
						elif member2_score_list.count(0) == 1:
				...
				for results in results_list:
					if results[0] == results_list[2*i][6]:
						opp_team_score += sum(results[2:5])

				team.finishing_process(opponent=[results_list[2*i][6]], score=sum(member1_score_list)+sum(member2_score_list), side=side, win=win, margin=sum(member1_score_list)+sum(member2_score_list)-opp_team_score)
...
def export_random_result(allocations, round_num):
	with open("private/Results"+str(round_num)+".csv", "w") as g:
		writer = csv.writer(g)
		writer.writerow(["team_name", "name", "R"+str(round_num)+" 1st", "R"+str(round_num)+" 2nd", "R"+str(round_num)+" rep", "Win", "Opponent", "Gov Opp"])

		for lattice in allocations:
			ga_base_score = random.randint(73, 77)
			gb_base_score = random.randint(73, 77)
			gc_base_score = random.randint(73,77)/2.0
			oa_base_score = random.randint(73, 77)
			ob_base_score = random.randint(73, 77)
			oc_base_score = random.randint(73,77)/2.0
			gov_members = copy.copy(lattice.grid.teams[0].debaters)
			opp_members = copy.copy(lattice.grid.teams[1].debaters)
			random.shuffle(gov_members)
			random.shuffle(opp_members)
			team1_win = 1 if sum([ga_base_score, gb_base_score, gc_base_score]) > sum([oa_base_score, ob_base_score, oc_base_score]) else 0
			...
			gc_score_from_panel1 = gc_base_score + random.randint(-2,+2)/2.0
			gc_score_from_panel2 = gc_base_score + random.randint(-2,+2)/2.0
			gc_score_from_chair = gc_base_score + random.randint(-2,+2)/2.0
			...
			oc_score_from_panel1 = oc_base_score + random.randint(-2,+2)/2.0
			oc_score_from_panel2 = oc_base_score + random.randint(-2,+2)/2.0
			oc_score_from_chair = oc_base_score + random.randint(-2,+2)/2.0
			...
			writer.writerow([lattice.grid.teams[0].name, gov_members[0].name, ga_score, 0, gc_score, team1_win, lattice.grid.teams[1].name, "gov"])
			writer.writerow([lattice.grid.teams[0].name, gov_members[1].name, 0, gb_score, 0, team1_win, lattice.grid.teams[1].name, "gov"])
			writer.writerow([lattice.grid.teams[1].name, opp_members[0].name, oa_score, 0, oc_score, 1-team1_win, lattice.grid.teams[0].name, "opp"])
			writer.writerow([lattice.grid.teams[1].name, opp_members[1].name, 0, ob_score, 0, 1-team1_win, lattice.grid.teams[0].name, "opp"])
...
def export_results(team_list, adjudicator_list):
		...
		for debater in team.debaters:
			...
			for score_list in debater.score_lists:
				if score_list.count(0) == 2:
					...
					sd += (sum(score_list)*2/3-average_score)**2
			if len(debater.score_lists) != 0:
	...
	for i in range(int((len(individual_score_rows[0])-2)/4)):
		header.extend(["round"+str(i+1)+" 1st", "round"+str(i+1)+" 2nd", "round"+str(i+1)+" reply", "round"+str(i+1)+" average"])
...
def check_results(filename):
	...
	for row in reader:
		...
		results_lists.append([row[0], row[1], float(row[2]), float(row[3]), float(row[4]), int(row[5]), row[6], row[7]])
	...
	for i, results_list in enumerate(results_lists):
		if results_list[2:5].count(0) == 0 or results_list[2:5].count(0) == 3:
	...
	opponents = {results_list[0]:results_list[6] for results_list in results_lists}
	opponents2 = {results_list[6]:results_list[0] for results_list in results_lists}
